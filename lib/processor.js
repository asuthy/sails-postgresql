...
Processor.prototype.castValue = function(table, key, value, attributes) {
    ...
    Object.keys(this.schema.attributes).forEach(function(attribute) {
        if (self.schema.attributes[attribute].columnName === key) {
            attr = attribute;
            return;
        }
    });
    ...
    // Lookup Schema "Type"
    if (!this.schema || !this.schema.attributes[attr]) return;
    var type;

    if (!_.isPlainObject(this.schema.attributes[attr])) {
        type = this.schema.attributes[attr];
    } else {
