...
module.exports = (function() {
    ...
    var adapter = {
        ...
        registerConnection: function(connection, collections, cb) {
            ...
            this.populateConnectionDetails(connections[connection.identity], connection.identity, function() {
        ...
        populateConnectionDetails: function(connection, connectionName, cb) {
            if (!connection.tables) {
                spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
                    var schemaName = getSchema(connectionName, Object.keys(connection.collections)[0]);
                    ...
                    var query = "SELECT x.relname, x.nspname || '.' || x.relname as \"Table\", x.attnum as \"#\", x.attname as \"Column\", x.\"Type\"," +
                        ...
                        "where x.nspname = '" + schemaName + "' order by 1,2;";
        ...
        describe: function(connectionName, table, cb) {
            spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
                ...
                var tableName = table.substring(0, 63);

                var tableSchema = [];

                connectionObject.tables.forEach(function(column) {
                    if (column.relname === tableName) {
                        tableSchema.push(column);
                    }
                });
                ...
                connectionObject.autoInc.forEach(function(row) {
                ...
                // Normalize Schema
                var normalizedSchema = utils.normalizeSchema(tableSchema);
